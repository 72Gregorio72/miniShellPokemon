...
/*   Updated: 2025/05/21 15:26:08 by gpicchio         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	exec_builtin(t_gen *gen, t_lexing *node)
{
	...
	else if (ft_strncmp("poke", node->command[0], ft_strlen("poke")) == 0
		&& ft_strlen(node->command[0]) == ft_strlen("poke"))
		return (ft_pokemon(gen), 1);
	else
